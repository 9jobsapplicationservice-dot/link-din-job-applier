...
portal_ai_answer_prompt = """
You are filling a job application form field.
Return only the final answer text with no explanation.

Rules:
1. For Yes/No style prompts, return exactly "Yes" or "No".
2. For numeric prompts (experience, salary, notice, years), return only a number.
3. For short text fields, keep answer under 120 characters.
4. For textarea or cover-letter style fields, keep answer concise and under 350 characters.
5. If options are provided, choose one option exactly as written.
6. Never include markdown, quotes, bullet points, or labels.
