...
def ai_answer_portal_question(
    ...
    """
    Generates constrained answers for external portal form fields.
    """
    print_lg("-- ANSWERING EXTERNAL PORTAL QUESTION using AI")
    try:
        options_text = ", ".join(options) if options else "N/A"
        prompt = portal_ai_answer_prompt.format(
            user_information_all or "N/A",
            question or "N/A",
            question_type,
            options_text
        )
        ...
        ai_error_alert(f"Error occurred while answering external portal question. {apiCheckInstructions}", e)
