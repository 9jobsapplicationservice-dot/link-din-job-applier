        ...
        with open(PATH + 'all_applied_applications_history.csv', 'r', encoding='utf-8') as file:
            ...
            for row in reader:
                jobs.append({
                    ...
                    'Apply_Type': row.get('Apply Type', ''),
                    'External_Portal': row.get('External Portal', ''),
                    'External_Apply_Status': row.get('External Apply Status', ''),
                    'External_Notes': row.get('External Notes', ''),
                })
        return jsonify(jobs)
