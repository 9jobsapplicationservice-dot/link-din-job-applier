...
5. (Not needed if you set `stealth_mode = True` in `config/settings.py` ) Download and install the appropriate [Chrome Driver](https://googlechromelabs.github.io/chrome-for-testing/) for Google Chrome and paste it in the location Chrome was installed, visit https://googlechromelabs.github.io/chrome-for-testing/ to download.
   - Startup troubleshooting: if you see `SessionNotCreatedException` with version mismatch like `ChromeDriver supports 146` but browser is `145`, the bot now tries UC auto mode, UC forced-major mode, and then falls back to standard Selenium ChromeDriver for reliability.
   - If you see `Could not remove old devtools port file`, it usually means stale profile locks; the bot now uses unique guest temp profiles per run and cleans known transient lock files.
  <br> <br>
  ***OR*** 
  <br> <br>
...
### Code Guidelines
  ...
  2. Must have variable setting explanation, and examples of valid values. Examples:
      ...
      ```
      ```python
      # Enforce strict title matching before applying? If True, title must contain all words from at least one search_terms entry.
      # Example: "Construction Site Coordinator" matches "Site Coordinator"; "Logistics and Warehouse Coordinator" does not.
      enforce_search_term_title_match = True     # True or False, Note: True or False are case-sensitive
      ```
      ```python
      # Enforce target location match before applying? If True, bot skips jobs whose work location does not match your primary search city.
      enforce_target_location_match = True     # True or False, Note: True or False are case-sensitive
      ```
      ```python
      # Require successful filter apply before processing jobs?
      # If True and LinkedIn filter UI fails (location/show-results), bot skips that search term to avoid wrong recommendation jobs.
      ...
      ```
      Note: LinkedIn can still show fallback/recommended jobs if filters fail to apply; use `enforce_target_location_match` + `strict_filter_apply_required` to block wrong-location applications.
      Note: In newer LinkedIn UI, filters may auto-apply via top chips without a modal `Show results` button; strict mode now treats this as valid when no modal submit is required.
      Note: Undetected Chrome startup now auto-detects your installed Chrome major version (no fixed major pin), reducing `session not created` mismatch issues.
